...
public class Arm
{
    ...
    public Arm()
    {
        ...
        ym2 = 374;
        r = 154.0;
        theta1 = -123*Math.PI/180.0; // initial angles of the upper arms
        theta2 = -72*Math.PI/180.0;
        valid_state = false;
    }
    ...
    public void directKinematic(){
        ...
        double d = Math.sqrt(((xm1-xm2)*(xm1-xm2))+((ym1-ym2)*(ym1-ym2)));//Math.sqrt(((xm1-xm2)*(xm1-xm2))+((ym1-ym2)*(ym1-ym2)));
        if (d<2*r){
            valid_state = true;
            // half distance between tool positions
            double  h = Math.sqrt((r*r)-((0.5*d)*(0.5*d)));
            double alpha= Math.atan((yj1-yj2)/(xj2-xj1));
            // tool position
            double xt = xa+h*Math.cos(Math.PI/2-alpha);      //i added these in,not completed not sure about alpha(james)
            double yt = ya+h*Math.sin(Math.PI/2-alpha);       //i added these in, not completed not sure about alpha(james)
    ...
    public void inverseKinematic(double xt_new,double yt_new){
        ...
        yt = yt_new;
        valid_state = true;
        double dx1 = xt - xm1; //distance from pen and motor
        double dy1 = yt - ym1;////distance from pen and motor

        double xa = xm1+0.5*(dx1);//middle of pen and motor
        double ya = ym1+0.5*(dy1) ;//middle of pen and motor
        double beta = Math.atan2(dy1,dx1) ;//beta for motor 1
        double alpha = ((Math.PI/2)-(Math.PI-beta));//alpha for motor 1
        ...
        double DTA = d1/2;//distance from pen to middle
        double h1 = Math.sqrt(r*r - d1*d1/4);
        // elbows positions
        double h =Math.sqrt((r*r)-(DTA*DTA));//lenght of distance of A to joint
        xj1 = xa+h*Math.cos(alpha);//position of joint 1
        yj1 = ya+h*Math.sin(alpha);//position of joint 1
        ...
        double dx2 = xt - xm2; //distance from pen and motor2
        double dy2 = yt - ym2; ////distance from pen and motor2
        double d2 = Math.sqrt(dx2*dx2+dy2*dy2);
        
        double x2a = xm2+0.5*(dx2);//middle of pen and motor2
        double y2a = ym2+0.5*(dy2) ;//middle of pen and motor2
        double beta2 = Math.atan2(dy2,dx2) ; // beta2 for motor 2
        double alpha2 = ((Math.PI/2)-(Math.PI-beta2));//alpha2 for motor 2
        ...
        double DTA2 = d2/2;//half distance of m2 to pen
        double newh2 = Math.sqrt((r*r)-(DTA2*DTA2)); //length of distance of x2a to joint

        double h2 = Math.sqrt(r*r - d2*d2/4);
        // elbows positions
        xj2 = x2a-newh2*Math.cos(alpha2);//position of joint 2
        yj2 = y2a-newh2*Math.sin(alpha2);//position of joint 2
